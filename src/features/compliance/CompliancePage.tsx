...
export default function CompliancePage() {
  ...
  // Governance case situation options
  const GOV_SITS = [
    { id: 'board-meetings', label: 'Board Meetings' }, { id: 'elections', label: 'Elections' },
    { id: 'bylaw-amendment', label: 'Bylaw / CC&R Amendment' }, { id: 'conflict-interest', label: 'Conflict of Interest' },
    { id: 'annual-budgeting', label: 'Annual Budgeting' }, { id: 'special-assessments', label: 'Special Assessments' },
    { id: 'covenant-violations', label: 'Covenant Violations' }, { id: 'governing-docs', label: 'Governing Doc Interpretation' },
    { id: 'record-requests', label: 'Record Requests' },
  ];
  ...
  const handleCreateCaseForMeeting = () => {
    if (!newCaseForm.title) { alert('Title required'); return; }
    const caseId = issues.createCase({ catId: newCaseForm.catId, sitId: newCaseForm.sitId, approach: 'self', title: newCaseForm.title, unit: 'N/A', owner: 'Board', priority: newCaseForm.priority as any, notes: `Linked to meeting: ${meetings.find(m => m.id === targetId)?.title || targetId}` });
    mtg.linkCase(targetId, caseId);
    setModal(null);
  };
        ...
        {expanded === m.id && (
          <div className="border-t border-ink-100 p-5 space-y-4">
            <div className="flex flex-wrap gap-2">
              ...
              <button onClick={() => openAddVote(m)} className="px-3 py-1.5 border border-ink-200 rounded-lg text-xs font-medium hover:bg-mist-50">+ Vote</button>
              ...
              <button onClick={() => { setTargetId(m.id); setLinkVoteId(''); setModal('linkVoteToMeeting'); }} className="px-3 py-1.5 border border-green-200 text-green-700 rounded-lg text-xs font-medium hover:bg-green-50">🔗 Link Vote</button>
            ...
            {/* Linked Cases */}
            {linkedCases.length > 0 && (<div><p className="font-bold text-ink-900 mb-2">📋 Linked Cases ({linkedCases.length})</p><div className="space-y-2">{linkedCases.map(c => (<div key={c.id} className="bg-violet-50 border border-violet-200 rounded-lg p-3 flex items-center justify-between"><div><p className="text-sm font-medium text-violet-900">{c.id}: {c.title}</p><p className="text-[10px] text-violet-600">{c.status} · {c.priority} · Created {c.created}</p></div><div className="flex gap-2"><button onClick={() => navigate('/issues')} className="text-[10px] text-accent-600 hover:underline">Open in Case Ops →</button><button onClick={() => mtg.unlinkCase(m.id, c.id)} className="text-xs text-red-400">×</button></div></div>))}</div></div>)}
            {/* Linked Votes */}
            {linkedVotes.length > 0 && (<div><p className="font-bold text-ink-900 mb-2">🗳 Linked Votes ({linkedVotes.length})</p><div className="space-y-2">{linkedVotes.map(v => { const r = elections.getResults(v.id, []); return (<div key={v.id} className="bg-green-50 border border-green-200 rounded-lg p-3 flex items-center justify-between"><div><p className="text-sm font-medium text-green-900">{v.title}</p><p className="text-[10px] text-green-600">{v.status} · {v.ballotItems.length} items · {v.ballots.length} ballots</p></div><div className="flex gap-2"><button onClick={() => navigate('/voting')} className="text-[10px] text-accent-600 hover:underline">Open in Voting →</button><button onClick={() => mtg.unlinkVote(m.id, v.id)} className="text-xs text-red-400">×</button></div></div>); })}</div></div>)}
    ...
    <div className="space-y-0">
      <div className="bg-gradient-to-r from-ink-900 via-ink-800 to-accent-800 rounded-t-xl p-8 text-white shadow-sm">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div><h2 className="font-display text-2xl font-bold">📋 Compliance & Governance</h2><p className="text-accent-200 text-sm mt-1">Runbook, meetings, filings & communications · {isDC ? 'District of Columbia' : jurisdiction} jurisdiction</p></div>
          <div className="flex items-center gap-6">
            ...
            <div className="text-center"><div className="text-4xl font-bold text-white">{mtgScore}%</div><div className="text-accent-200 text-xs">Meeting Compliance</div></div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-b-xl border-x border-b border-ink-100 p-6">
        ...
        {tab === 'filings' && (<div className="space-y-4">
          <div className="flex items-center justify-between"><div><h3 className="font-display text-lg font-bold text-ink-900">📅 Regulatory Filings & Deadlines</h3><p className="text-xs text-ink-400">Tax returns, government reports, inspections</p></div><button onClick={() => { setForm({ name: '', category: 'tax', dueDate: '', responsible: 'President', recurrence: 'annual', legalRef: '', notes: '' }); setModal('addFiling'); }} className="px-4 py-2 bg-ink-900 text-white rounded-lg text-sm font-medium hover:bg-ink-800">+ Add Filing</button></div>
          <div className="bg-white rounded-xl border border-ink-100 overflow-hidden divide-y divide-ink-50">
            {comp.filings.sort((a, b) => a.dueDate.localeCompare(b.dueDate)).map(fi => { const isPast = fi.status === 'pending' && new Date(fi.dueDate) < new Date(); return (<div key={fi.id} className={`p-4 ${fi.status === 'filed' ? 'bg-sage-50 bg-opacity-50' : isPast ? 'bg-red-50' : ''}`}><div className="flex items-start justify-between gap-3"><div className="flex-1"><div className="flex items-center gap-2 flex-wrap"><p className={`text-sm font-medium ${fi.status === 'filed' ? 'text-ink-500 line-through' : 'text-ink-900'}`}>{fi.name}</p><span className={`pill px-1.5 py-0.5 rounded text-xs ${fi.status === 'filed' ? 'bg-sage-100 text-sage-700' : isPast ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}>{fi.status === 'filed' ? '✓ Filed' : isPast ? 'OVERDUE' : 'Pending'}</span><span className="text-xs text-ink-400">{fi.category} · {fi.recurrence}</span></div><p className="text-xs text-ink-500 mt-1">Due: {fi.dueDate} · {fi.responsible}{fi.filedDate ? ` · Filed: ${fi.filedDate}` : ''}{fi.confirmationNum ? ` · Ref: ${fi.confirmationNum}` : ''}</p>{fi.notes && <p className="text-xs text-ink-400 mt-1">{fi.notes}</p>}{fi.legalRef && <p className="text-xs text-ink-300 font-mono mt-0.5">{fi.legalRef}</p>}<div className="mt-2 flex flex-wrap items-center gap-2">{fi.attachments.map(att => (<span key={att.name} className="inline-flex items-center gap-1.5 bg-mist-50 border border-mist-200 rounded-lg px-2.5 py-1"><span className="text-[11px] text-accent-600 font-medium">📎 {att.name}</span><span className="text-[10px] text-ink-400">{att.size}</span><button onClick={() => comp.removeFilingAttachment(fi.id, att.name)} className="text-red-400 hover:text-red-600 text-xs ml-1">✕</button></span>))}<button onClick={() => { setTargetId(fi.id); setPendingFile(null); setModal('addFilingAtt'); }} className="text-[11px] text-accent-600 font-medium hover:text-accent-700 border border-dashed border-accent-300 rounded-lg px-2.5 py-1 hover:bg-accent-50">+ Attach proof</button></div></div><div className="flex gap-2 shrink-0">{fi.status === 'pending' && <button onClick={() => { setTargetId(fi.id); setForm({ filedDate: new Date().toISOString().split('T')[0], confirmationNum: '' }); setModal('markFiled'); }} className="px-3 py-1 bg-sage-600 text-white rounded text-xs font-medium hover:bg-sage-700">Mark Filed</button>}<button onClick={() => { if (confirm('Remove?')) comp.deleteFiling(fi.id); }} className="text-xs text-red-400 hover:text-red-600">Remove</button></div></div></div>); })}
          </div>
        </div>)}

        {/* MEETINGS */}
        {tab === 'meetings' && (<div className="space-y-6">
          <div className="bg-mist-50 border border-mist-200 rounded-xl p-5"><div className="flex items-center justify-between mb-3"><div><h3 className="font-bold text-ink-900">Meeting Compliance — {jurisdiction}</h3><p className="text-xs text-ink-500 mt-0.5">Requirements from <span className="font-mono text-accent-700">{isDC ? 'DC Code § 29-1101 et seq.' : `${jurisdiction} Condo Act`}</span>{hasBylaws && <>, cross-referenced with <span className="font-semibold">Condominium Bylaws</span></>}{hasCCRs && <> and <span className="font-semibold">CC&Rs</span></>}{!hasBylaws && <span className="text-amber-600 ml-1">· ⚠ Upload Bylaws for bylaw-specific checks</span>}</p></div><button onClick={openAddMeeting} className="px-4 py-2 bg-ink-900 text-white rounded-lg text-sm font-medium hover:bg-ink-800">+ Schedule Meeting</button></div><div className="grid grid-cols-1 md:grid-cols-4 gap-3">{meetingReqs.map(r => { const met = (r as any).isPct ? r.actual >= r.req : r.actual >= r.req; return (<div key={r.label} className={`bg-white rounded-lg p-3 border ${met ? 'border-sage-200' : 'border-red-200'}`}><div className="flex justify-between items-center mb-1"><span className="font-bold text-ink-900 text-sm">{r.label}</span><span className={`text-lg ${met ? 'text-sage-600' : 'text-red-600'}`}>{met ? '✓' : '✗'}</span></div>{(r as any).isPct ? (<p className="text-xs text-ink-500">Rate: <strong className={met ? 'text-sage-600' : 'text-red-600'}>{r.actual}%</strong></p>) : (<p className="text-xs text-ink-500">Required: <strong>{r.req}</strong> · Actual: <strong className={met ? 'text-sage-600' : 'text-red-600'}>{r.actual}</strong></p>)}<p className="text-xs text-ink-400">{r.freq}</p><div className="mt-1.5 flex items-center gap-1"><span className="text-[9px] px-1 py-0.5 rounded bg-accent-50 text-accent-600 font-medium">{r.legalRef}</span></div></div>); })}</div></div>
      ...
      {/* Meeting modals */}
      {(modal === 'addMeeting' || modal === 'editMeeting') && (<Modal title={modal === 'addMeeting' ? 'Schedule Meeting' : 'Edit Meeting'} onClose={() => setModal(null)} onSave={saveMeeting} saveLabel={modal === 'addMeeting' ? 'Schedule' : 'Save'}><div className="space-y-3"><div><label className="block text-xs font-medium text-ink-700 mb-1">Title *</label><input value={mForm.title} onChange={e => setMForm({ ...mForm, title: e.target.value })} className="w-full px-3 py-2 border border-ink-200 rounded-lg text-sm" placeholder="February Board Meeting" /></div><div className="grid grid-cols-2 gap-3"><div><label className="block text-xs font-medium text-ink-700 mb-1">Type</label><select value={mForm.type} onChange={e => setMForm({ ...mForm, type: e.target.value })} className="w-full px-3 py-2 border border-ink-200 rounded-lg text-sm">{['BOARD','ANNUAL','QUARTERLY','SPECIAL','EMERGENCY'].map(t => <option key={t}>{t}</option>)}</select></div><div><label className="block text-xs font-medium text-ink-700 mb-1">Status</label><select value={mForm.status} onChange={e => setMForm({ ...mForm, status: e.target.value })} className="w-full px-3 py-2 border border-ink-200 rounded-lg text-sm">{['SCHEDULED','COMPLETED','CANCELLED','RESCHEDULED'].map(s => <option key={s}>{s}</option>)}</select></div></div><div className="grid grid-cols-2 gap-3"><div><label className="block text-xs font-medium text-ink-700 mb-1">Date *</label><input type="date" value={mForm.date} onChange={e => setMForm({ ...mForm, date: e.target.value })} className="w-full px-3 py-2 border border-ink-200 rounded-lg text-sm" /></div><div><label className="block text-xs font-medium text-ink-700 mb-1">Time</label><input type="time" value={mForm.time} onChange={e => setMForm({ ...mForm, time: e.target.value })} className="w-full px-3 py-2 border border-ink-200 rounded-lg text-sm" /></div></div><div className="grid grid-cols-2 gap-3"><div><label className="block text-xs font-medium text-ink-700 mb-1">Location</label><input value={mForm.location} onChange={e => setMForm({ ...mForm, location: e.target.value })} className="w-full px-3 py-2 border border-ink-200 rounded-lg text-sm" /></div><div><label className="block text-xs font-medium text-ink-700 mb-1">Virtual Link</label><input value={mForm.virtualLink} onChange={e => setMForm({ ...mForm, virtualLink: e.target.value })} className="w-full px-3 py-2 border border-ink-200 rounded-lg text-sm" placeholder="https://zoom.us/..." /></div></div><div><label className="block text-xs font-medium text-ink-700 mb-1">Agenda (one per line)</label><textarea value={mForm.agenda} onChange={e => setMForm({ ...mForm, agenda: e.target.value })} className="w-full px-3 py-2 border border-ink-200 rounded-lg text-sm" rows={4} placeholder={"Review January financials\nElevator maintenance proposal"} /></div><div><label className="block text-xs font-medium text-ink-700 mb-1">Notes</label><input value={mForm.notes} onChange={e => setMForm({ ...mForm, notes: e.target.value })} className="w-full px-3 py-2 border border-ink-200 rounded-lg text-sm" /></div></div></Modal>)}
      ...
      {/* Link existing vote to meeting */}
      {modal === 'linkVoteToMeeting' && (<Modal title="Link Vote / Election" onClose={() => setModal(null)} onSave={() => { if (linkVoteId) { mtg.linkVote(targetId, linkVoteId); setModal(null); } }} saveLabel="Link"><div className="space-y-3"><p className="text-xs text-ink-500">Select a vote from Votes & Resolutions to link to this meeting.</p><select value={linkVoteId} onChange={e => setLinkVoteId(e.target.value)} className="w-full px-3 py-2 border border-ink-200 rounded-lg text-sm"><option value="">Select a vote...</option>{elections.elections.map(v => <option key={v.id} value={v.id}>{v.title} ({v.status}) — {v.ballotItems.length} items</option>)}</select>{linkVoteId && (() => { const v = elections.elections.find(x => x.id === linkVoteId); return v ? <div className="bg-green-50 border border-green-200 rounded-lg p-3"><p className="text-xs text-green-800"><strong>{v.title}</strong> · {v.status} · {v.ballotItems.map(bi => bi.title).join(', ')}</p></div> : null; })()}</div></Modal>)}
