        ...
        return (<Modal title={runbookAction === 'case' ? 'Add Case' : 'Schedule Meeting'} onClose={() => setModal(null)} onSave={() => setModal(null)} saveLabel="Done"><div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <button onClick={() => {
              ...
              else { const mType = item?.meetingType || 'BOARD'; const agenda = item?.suggestedAgenda || [item?.task || 'Agenda item']; mtg.addMeeting({ title: `${item?.task || 'Meeting'}`, type: mType, status: 'SCHEDULED', date: '', time: '19:00', location: 'Community Room', virtualLink: '', agenda, notes: `From Runbook: ${item?.task || ''}. ${item?.legalRef || ''}` }); comp.toggleCompletion(targetId); setModal(null); setTab('meetings'); }
