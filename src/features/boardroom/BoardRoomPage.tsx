...
type ModalType = null | 'addMeeting' | 'editMeeting' | 'attendees' | 'minutes' | 'addVote' | 'linkCaseToMeeting' | 'linkVoteToMeeting' | 'createCaseForMeeting';
...
export default function BoardRoomPage() {
  ...
  return (
    <div className="space-y-0">
      ...
      {modal === 'linkVoteToMeeting' && (<Modal title="Link Vote / Election" onClose={() => setModal(null)} onSave={() => { if (linkVoteId) { mtg.linkVote(targetId, linkVoteId); setModal(null); } }} saveLabel="Link"><div className="space-y-3"><p className="text-xs text-ink-500">Select a vote from Votes & Resolutions to link to this meeting.</p><select value={linkVoteId} onChange={e => setLinkVoteId(e.target.value)} className="w-full px-3 py-2 border border-ink-200 rounded-lg text-sm"><option value="">Select a vote...</option>{elections.elections.map(v => <option key={v.id} value={v.id}>{v.title} ({v.status}) — {v.ballotItems.length} items</option>)}</select></div></Modal>)}
