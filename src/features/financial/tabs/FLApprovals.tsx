...
export default function FLApprovals() {
  ...
  const financialContext = useMemo(() => getFinancialContext(financialStore), [financialStore]);

  // ─── Compute funding analysis for a given amount ───────────
  const analyzeAmount = (amount: number) => analyzeFunding(amount, financialContext);
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-4">
          {filtered.map(approval => {
            ...
            const analysis = isExpanded ? analyzeAmount(approval.amount) : null;
